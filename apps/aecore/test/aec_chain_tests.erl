...
out_of_order_test_block_chain() ->
    ...
    TxsFun = fun(1) ->
                     ...
                     Tx3 = make_spend_tx(PubKey, 3, PubKey, 1),
                     [aec_test_utils:sign_tx(Tx1, PrivKey),
                      aec_test_utils:sign_tx(Tx2, PrivKey),
                      aec_test_utils:sign_tx(Tx3, PrivKey)
                     ];
                (_) ->
                     []
             end,
    ...
    ?assertEqual(false, aec_blocks:is_key_block(MB3)),
    ...
    %% We must have the key block for a micro block
    ?assertMatch({error, {illegal_orphan, _}}, insert_block(MB1)),
    ?assertMatch({error, {illegal_orphan, _}}, insert_block(MB2)),
    ?assertMatch({error, {illegal_orphan, _}}, insert_block(MB3)),

    %% If we have the micro block, the blocks can be out of order
    ?assertMatch(ok, insert_block(KB1)),
    ?assertMatch(ok, insert_block(MB3)),
    ?assertMatch(ok, insert_block(MB2)),

    %% But as long as we do not have a full chain, the key block is the top
    ?assertEqual(KB1Hash, top_block_hash()),

    %% Connect the chain with the missing micro block
    ?assertMatch(ok, insert_block(MB1)),
    ?assertEqual(MB3Hash, top_block_hash()),

    %% Now we can add the top key block
    ?assertMatch(ok, insert_block(KB2)),
...
key_hash_orphaned_chain() ->
    ...
    %% Assert that we do have an orphaned chain.
    ?assertEqual(true, aec_chain:hash_is_connected_to_genesis(KB1Hash)),
    ?assertEqual(false, aec_chain:hash_is_connected_to_genesis(MB2Hash)),
    ?assertEqual(false, aec_chain:hash_is_connected_to_genesis(MB3Hash)),
    ...
    %% Insert KB2 at the top.
    ok = insert_block(KB2),

    %% KB2 is now our top
    ?assertEqual(KB2Hash, aec_chain:top_block_hash()),
    ...
    {ok, MB3KeyHash} = aec_chain:get_key_hash(MB3Hash),
    {ok, KB2KeyHash} = aec_chain:get_key_hash(KB2Hash),

    ?assertEqual(KB1KeyHash, KB1Hash),
    ?assertEqual(MB1KeyHash, KB1Hash),
    ?assertEqual(MB2KeyHash, KB1Hash),
    ?assertEqual(MB3KeyHash, KB1Hash),
